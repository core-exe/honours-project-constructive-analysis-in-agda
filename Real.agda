...
K : ℝ -> ℕ
K x = let p = ↥ (ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ); q = ↧ₙ (ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ) in suc ℤ.∣ p /ℕ q ∣

private
  abstract
    Kx=1+t : ∀ x -> + K x ≡ + 1 ℤ.+ ((↥ (ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ) /ℕ ↧ₙ (ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ)))
    Kx=1+t x = let t = (↥ (ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ) /ℕ ↧ₙ (ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ)) in begin-equality
      + K x             ≡⟨ _≡_.refl ⟩
      + 1 ℤ.+ + ℤ.∣ t ∣ ≡⟨ cong (λ x -> + 1 ℤ.+ x) (ℤP.0≤i⇒+∣i∣≡i (0≤n⇒0≤n/ℕd (↥ (ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ)) (↧ₙ (ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ))
                                (ℚP.≥0⇒↥≥0 {numerator (ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ)} {denominator-1 (ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ)} (ℚP.≤-trans (ℚP.0≤∣p∣ (seq x 1)) (
                                ℚP.≤-trans (ℚP.p≤p+q ℚ.∣ seq x 1 ∣ 2ℚᵘ) (ℚP.≤-reflexive-≡ (ℚP.↥↧≡⇒≡ refl refl))))))) ⟩
      + 1 ℤ.+ t          ∎
      where
        open ℤP.≤-Reasoning
...
abstract
  ...
  canonical-well-defined x = left , right
    where
      ...
      left = let t = ↥ (ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ) /ℕ ↧ₙ (ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ) in begin-strict
        ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ <⟨ proj₁ (proj₂ (least-ℤ>ℚ (ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ))) ⟩
        (+ 1 ℤ.+ t) / 1       ≃⟨ ℚP.≃-reflexive (ℚP./-cong (sym (Kx=1+t x)) refl) ⟩
        + K x / 1              ∎
        where open ℚP.≤-Reasoning

      right : ∀ (n : ℤ) -> ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ ℚ.< n / 1 -> + K x ℤ.≤ n
      right n hyp = let t = ↥ (ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ) /ℕ ↧ₙ (ℚ.∣ seq x 1 ∣ ℚ.+ 2ℚᵘ) in begin
        + K x     ≡⟨ Kx=1+t x ⟩
...
reg (x * y) (suc k₁) (suc k₂) = let m = suc k₁; n = suc k₂; k = K x ℕ.⊔ K y; 2km = 2 ℕ.* k ℕ.* m; 2kn = 2 ℕ.* k ℕ.* n
  ...
  ℚ.∣ y₂ₖₙ ∣ ℚ.* ℚ.∣ x₂ₖₘ ℚ.- x₂ₖₙ ∣           ≤⟨ ℚP.+-mono-≤
                                                  (ℚP.*-monoˡ-≤-nonNeg (ℚ.∣ y₂ₖₘ ℚ.- y₂ₖₙ ∣) {{ℚP.∣-∣-nonNeg (y₂ₖₘ ℚ.- y₂ₖₙ)}} ∣x₂ₖₘ∣≤k)
                                                  (ℚP.*-monoˡ-≤-nonNeg (ℚ.∣ x₂ₖₘ ℚ.- x₂ₖₙ ∣) {{ℚP.∣-∣-nonNeg (x₂ₖₘ ℚ.- x₂ₖₙ)}} ∣y₂ₖₙ∣≤k) ⟩
  (+ k / 1) ℚ.* ℚ.∣ y₂ₖₘ ℚ.- y₂ₖₙ ∣ ℚ.+
  (+ k / 1) ℚ.* ℚ.∣ x₂ₖₘ ℚ.- x₂ₖₙ ∣           ≤⟨ ℚP.+-mono-≤
                                                 (ℚP.*-monoʳ-≤-nonNeg (+ k / 1) (fast-reg y 2km 2kn))
                                                 (ℚP.*-monoʳ-≤-nonNeg (+ k / 1) (fast-reg x 2km 2kn)) ⟩
  (+ k / 1) ℚ.* ((+ 1 / 2km) ℚ.+
  (+ 1 / 2kn)) ℚ.+
  (+ k / 1) ℚ.* ((+ 1 / 2km) ℚ.+
...
data NonNegative : Pred ℝ 0ℓ where
  nonNeg* : ∀ {x} -> (∀ (n : ℕ) -> {{n≢0 : n ≢0}} -> seq x n ℚ.≥ ℚ.- ((+ 1 / n))) -> NonNegative x
