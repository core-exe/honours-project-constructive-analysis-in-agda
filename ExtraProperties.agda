...
archimedean-ℚ (mkℚᵘ +[1+ g ] q-1) (mkℚᵘ u v-1) posp = let p = suc g; q = suc q-1; v = suc v-1
  ...
  + ℤ.∣ + 1 ℤ.+ t ∣ ℤ.* (+ p ℤ.* + v) ≤⟨ ℤP.*-monoʳ-≤-nonNeg (+ (p ℕ.* v)) (ℤ.+≤+ (ℤP.∣i+j∣≤∣i∣+∣j∣ (+ 1) t)) ⟩
...
q≤r⇒+p/r≤+p/q : ∀ p q r -> {{q≢0 : q ≢0}} -> {{r≢0 : r ≢0}} -> q ℕ.≤ r -> (+ p / r) ℚ.≤ (+ p / q)
q≤r⇒+p/r≤+p/q p (suc k₁) (suc k₂) q≤r = ℚ.*≤* (ℤP.*-monoˡ-≤-nonNeg (+ p) (ℤ.+≤+ q≤r))


q<r⇒+p/r<+p/q : ∀ p q r -> {p ≢0} -> {{q≢0 : q ≢0}} -> {{r≢0 : r ≢0}} -> q ℕ.< r -> (+ p / r) ℚ.< (+ p / q)
q<r⇒+p/r<+p/q (suc k₁) (suc k₂) (suc k₃) q<r = ℚ.*<* (ℤP.*-monoˡ-<-pos (+ (suc k₁)) (ℤ.+<+ q<r))

p≤q⇒p/r≤q/r : ∀ (p q : ℤ) -> ∀ (r : ℕ) -> {{r≢0 : r ≢0}} -> p ℤ.≤ q -> (p / r) ℚ.≤ (q / r)
p≤q⇒p/r≤q/r p q (suc k₁) p≤q = ℚ.*≤* (ℤP.*-monoʳ-≤-nonNeg (+ (suc k₁)) p≤q)

p<q⇒p/r<q/r : (p q : ℤ) (r : ℕ) {{r≢0 : r ≢0}} → p ℤ.< q → (p / r) ℚ.< (q / r)
p<q⇒p/r<q/r p q (suc r-1) p<q = ℚ.*<* (ℤP.*-monoʳ-<-pos (+ (suc r-1)) p<q)
...
ℚ-*-mono-≤ {p} {q} {r} {s} nonp nonr p≤q r≤s = begin
  p ℚ.* r ≤⟨ ℚP.*-monoˡ-≤-nonNeg r {{nonr}} p≤q ⟩
  q ℚ.* r ≤⟨ ℚP.*-monoʳ-≤-nonNeg q {{(ℚ.nonNegative (ℚP.≤-trans (ℚP.nonNegative⁻¹ p {{nonp}}) p≤q))}} r≤s ⟩
  q ℚ.* s  ∎
  where open ℚP.≤-Reasoning
...
p+q>r⇒p>2⁻¹r∨q>2⁻¹r p q r p+q>r = [ (λ hyp -> inj₁ (lem hyp)) , (λ hyp -> inj₂ hyp) ]′ (≤∨> q ((+ 1 / 2) ℚ.* r))
  where
    ...
    lem hyp = begin-strict
      (+ 1 / 2) ℚ.* r                             ≃⟨ solve 1 (λ r → (Κ (+ 1 / 2) ⊗ r) ⊜ (r ⊖ (Κ (+ 1 / 2)) ⊗ r)) ℚP.≃-refl r ⟩
...
p≤q+j⁻¹⇒p≤q : ∀ {p q} -> (∀ j -> {{j≢0 : j ≢0}} -> p ℚ.≤ q ℚ.+ (+ 1 / j)) -> p ℚ.≤ q
